...
\begin{document}
	...
	\begin{ExamProblems}
		...
		\subsection*{C2}
		The sample statistics are $\bar{x}_A = 19.5$, $s_A \approx 1.87$, $\bar{x}_B \approx 24.33$, and $s_B \approx 1.53$, while the population parameters are $\mu$, $\sigma_A = 3.5$, and $\sigma_B = 4.0$. Both samples come from the same population, so each sample provides an estimate for $\mu$.
		...
		\]
		With known $\sigma_B = 4.0$ and $n_B = 3$,
		\[
		SE_B = \frac{\sigma_B}{\sqrt{n_B}}
		\]
		\[
		SE_B = \frac{4.0}{\sqrt{3}} \approx 2.31
		\]
		\[
		E_B = z_{0.025} \cdot SE_B
		\]
		\[
		E_B = 1.96(2.31) \approx 4.52
		\]
		\[
		\mu \in \bar{x}_B \pm E_B
		\]
		\[
		\mu \in 24.33 \pm 4.52 = [24.33 - 4.52, 24.33 + 4.52] = [19.81, 28.85]
		\]

		\subsection*{C4}
		We are 95\% confident the true mean weekly subsidy in the population is between about 16.70 and 22.30 dollars based on Sample A, and 95\% confident it is between about 19.81 and 28.85 dollars based on Sample B. The intervals reflect uncertainty from different samples drawn from the same population.
		...
		The grouped data pairs (orders per hour, frequency) are \(14 \rightarrow 8\), \(18 \rightarrow 16\), \(22 \rightarrow 20\), and \(26 \rightarrow 10\).
		...
		A sample of 67 loading times is grouped into class intervals below. The firm has a historical estimate of the population standard deviation, \(\sigma = 5.8\) minutes.
		The grouped intervals and frequencies are 20--24 minutes (12), 25--29 minutes (18), 30--34 minutes (20), and 35--39 minutes (17).
		...
		\]
		\[
		\sum f x = 12(22) + 18(27) + 20(32) + 17(37) = 2019
		\]
		\[
		\bar{x} = \frac{2019}{67} \approx 30.13
		\]
		Second, we calculate the sample standard deviation $s$.
		\[
		s^2 = \frac{\sum f(x-\bar{x})^2}{n-1}
		\]
		\[
		\sum f(x-\bar{x})^2 \approx 12(22-30.13)^2 + 18(27-30.13)^2 + 20(32-30.13)^2 + 17(37-30.13)^2
		\]
		\[
		\sum f(x-\bar{x})^2 \approx 1841.79
		\]
		\[
		s = \sqrt{\frac{\sum f(x-\bar{x})^2}{n-1}}
		\]
		\[
		s = \sqrt{\frac{1841.79}{67-1}} \approx 5.28
		\]

		\subsection*{C2}
		The sample statistics are $\bar{x} \approx 30.13$ and $s \approx 5.28$, while the population parameters are $\mu$ and $\sigma = 5.8$. The sample values estimate the population values, and $\mu$ remains unknown.

		\subsection*{C3}
		Even with 67 observations, a point estimate does not show how much the mean could vary. A confidence interval summarizes that uncertainty by giving a plausible range for $\mu$.

		\subsection*{C5}
		With known $\sigma = 5.8$ and $n=67$,
		\[
		SE = \frac{\sigma}{\sqrt{n}}
		\]
		\[
		SE = \frac{5.8}{\sqrt{67}} \approx 0.71
		\]
		For 95\% confidence, $z_{0.025}=1.96$.
		\[
		E = z_{0.025} \cdot SE
		\]
		\[
		E = 1.96(0.71) \approx 1.39
		\]
		\[
		\mu \in \bar{x} \pm E
		\]
		\[
		\mu \in 30.13 \pm 1.39 = [30.13 - 1.39, 30.13 + 1.39] = [28.74, 31.52]
		\]

		\subsection*{C4}
		We are 95\% confident the true mean loading time is between about 28.74 and 31.52 minutes. The interval provides a plausible range for the population mean based on the sample.
	\end{ExamProblems}
\end{document}
