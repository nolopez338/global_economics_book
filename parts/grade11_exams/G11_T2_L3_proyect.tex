...
\begin{document}
	...
	\begin{multicols}{2}
		...
		\textbf{1.6 Suggested workflow (C10 $\rightarrow$ C8/C9)}\par
		\begin{enumerate}
			...
			\item Use a macro to save the current scenario and repeat for another case.
		\end{enumerate}
		...
		\textbf{Dashboard justification}\par
		The confidence-level dropdown and scenario selector update the CI band chart and the KPI cards ($\bar{x}$, $ME$, interval bounds). The visual overlap of daily values with the CI band justifies the conclusion reported to the desk manager.
		
		\textbf{Macro use}\par
		A \texttt{SnapshotScenario()} macro stores the current scenario, confidence level, and CI outputs in the comparison table. This allows immediate repetition for an alternative week without reformatting the workbook.
		...
		\textbf{Dashboard justification}\par
		A scenario slicer compares \textit{Port A -- Q2} vs. \textit{Port B -- Q2}. The CI comparison chart and KPI tiles show interval width and center for each route, supporting a justified scenario-based conclusion.
		
		\textbf{Macro use}\par
		A \texttt{ResetAndRecompute()} macro clears the input range, imports the next scenario block, recalculates outputs, and logs both CI results in a comparison table for rapid reporting.
