...
\begin{document}
	...
	\begin{ExamProblems}
		\item
		\subsection*{Problem description}
		A microfinance institution records approval times (days) for 15 recent small-business loan applications:
		\[
		4.2,\ 5.1,\ 4.8,\ 6.0,\ 5.4,\ 4.9,\ 5.7,\ 4.5,\ 5.0,\ 6.2,\ 4.7,\ 5.3,\ 5.6,\ 4.4,\ 5.8
		\]
		Construct a 95\% confidence interval for the population mean approval time and provide an economic conclusion for operations planning.
		
		\subsection*{C10}
		Step 1: Organize raw data and compute descriptive statistics
		\[
		n=15,\qquad \bar{x}=\frac{\sum x_i}{n}=\frac{77.6}{15}=5.17\text{ days}
		\]
		\[
		s=\sqrt{\frac{\sum (x_i-\bar{x})^2}{n-1}}\approx 0.61\text{ days}
		\]
		Step 2: Identify parameter and distributional assumptions
		\[
		\mu=\text{population mean loan approval time}
		\]
		Because $\sigma$ is unknown, use the t distribution with
		\[
		df=n-1=14.
		\]
		Step 3: Compute standard error and critical value
		\[
		SE=\frac{s}{\sqrt{n}}=\frac{0.61}{\sqrt{15}}\approx 0.16,\qquad t^*=t_{0.025,14}\approx 2.145
		\]
		Step 4: Construct the confidence interval
		\[
		\mu\in \bar{x}\pm t^*SE=5.17\pm 2.145(0.16)=5.17\pm 0.34=[4.83,\,5.51]
		\]
		Step 5: Economic/financial interpretation and conclusion
		With 95\% confidence, the institution's mean approval time is between 4.83 and 5.51 days.
		Operationally, staffing and customer communication should be based on an expected approval time near 5 days, not below 4 days.
		
		\newpage
		\item
		\subsection*{Problem description}
		An investment desk records daily return rates (\%) for 18 trading days of a balanced fund:
		\[
		0.32,\ 0.41,\ 0.28,\ 0.36,\ 0.39,\ 0.44,\ 0.35,\ 0.31,\ 0.47,\ 0.40,\ 0.38,\ 0.33,\ 0.42,\ 0.29,\ 0.37,\ 0.45,\ 0.34,\ 0.43
		\]
		Construct a 90\% confidence interval for the population mean daily return and conclude for portfolio communication.
		
		\subsection*{C10}
		Step 1: Organize raw data and compute descriptive statistics
		\[
		n=18,\qquad \bar{x}=\frac{\sum x_i}{n}=\frac{6.74\%}{18}=0.374\%
		\]
		\[
		s=\sqrt{\frac{\sum (x_i-\bar{x})^2}{n-1}}\approx 0.056\%
		\]
		Step 2: Identify parameter and distributional assumptions
		\[
		\mu=\text{population mean daily return of the fund}
		\]
		The population standard deviation is unknown, so we use a t interval with
		\[
		df=n-1=17.
		\]
		Step 3: Compute standard error and critical value
		\[
		SE=\frac{0.056\%}{\sqrt{18}}\approx 0.013\%,\qquad t^*=t_{0.05,17}\approx 1.740
		\]
		Step 4: Construct the confidence interval
		\[
		\mu\in 0.374\%\pm 1.740(0.013\%)=0.374\%\pm 0.023\%=[0.351\%,\,0.397\%]
		\]
		Step 5: Economic/financial interpretation and conclusion
		With 90\% confidence, the mean daily return is between 0.351\% and 0.397\%.
		For communication with investors, a central expectation near 0.37\% per day is supported by these data.
		
		\newpage
		\item
		\subsection*{Problem description}
		A digital payments company records merchant transaction fees (USD) for 12 recent transactions:
		\[
		2.10,\ 2.35,\ 2.28,\ 2.42,\ 2.18,\ 2.50,\ 2.31,\ 2.27,\ 2.46,\ 2.22,\ 2.39,\ 2.33
		\]
		Construct a 99\% confidence interval for the population mean transaction fee and conclude for pricing policy.
		
		\subsection*{C10}
		Step 1: Organize raw data and compute descriptive statistics
		\[
		n=12,\qquad \bar{x}=\frac{\sum x_i}{n}=\frac{27.81}{12}=2.318\ \text{USD}
		\]
		\[
		s=\sqrt{\frac{\sum (x_i-\bar{x})^2}{n-1}}\approx 0.117\ \text{USD}
		\]
		Step 2: Identify parameter and distributional assumptions
		\[
		\mu=\text{population mean transaction fee}
		\]
		Since the population variance is unknown, use the t distribution with
		\[
		df=n-1=11.
		\]
		Step 3: Compute standard error and critical value
		\[
		SE=\frac{0.117}{\sqrt{12}}\approx 0.034,\qquad t^*=t_{0.005,11}\approx 3.106
		\]
		Step 4: Construct the confidence interval
		\[
		\mu\in 2.318\pm 3.106(0.034)=2.318\pm 0.105=[2.213,\,2.423]\ \text{USD}
		\]
		Step 5: Economic/financial interpretation and conclusion
		With 99\% confidence, the mean merchant fee is between 2.213 and 2.423 USD.
		For pricing policy, the estimate is centered near 2.32 USD and suggests average fees are close to the upper end of the firm's current pricing band.
	\end{ExamProblems}
\end{document}
