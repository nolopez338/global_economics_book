...
\begin{document}
	...
	\begin{itemize}
		...
		\item \hyperlink{problem1-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	– High traffic → 42 thousand USD
	– Low traffic → 8 thousand USD
	
	
	
	
	• \textbf{Keep the current menu}, with profit outcomes:
	– High traffic → 28 thousand USD
	– Low traffic → 20 thousand USD
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Launch}, S\_i) \&= \max\{42, 8\} = 42 \\
		\max\_i \text{Payoff}(\text{Keep}, S\_i) \&= \max\{28, 20\} = 28
	\end{aligned}
	\]
	\[
	\max\{42, 28\} = 42
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Launch}, S\_i) \&= \min\{42, 8\} = 8 \\
		\min\_i \text{Payoff}(\text{Keep}, S\_i) \&= \min\{28, 20\} = 20
	\end{aligned}
	\]
	\[
	\max\{8, 20\} = 20
	...
	\[
	\begin{aligned}
		EV\_\{\text{Launch}\} \&= 0.55(42) + 0.45(8) = 23.1 + 3.6 = 26.7 \\
		EV\_\{\text{Keep}\} \&= 0.55(28) + 0.45(20) = 15.4 + 9 = 24.4
	\end{aligned}
	\]
	\[
	\max\{26.7, 24.4\} = 26.7
	...
	\subsection*{(C1) Formulating the decision problem for an optimal choice}
	...
	\begin{itemize}
		...
		\item \hyperlink{problem2-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	• Wind turbines → 90 (Windy), 45 (Sunny), 30 (Cloudy)
	• Solar farms → 35 (Windy), 95 (Sunny), 25 (Cloudy)
	• Hybrid system → 70 (Windy), 65 (Sunny), 60 (Cloudy)
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Wind}, S\_i) \&= \max\{90, 45, 30\} = 90 \\
		\max\_i \text{Payoff}(\text{Solar}, S\_i) \&= \max\{35, 95, 25\} = 95 \\
		\max\_i \text{Payoff}(\text{Hybrid}, S\_i) \&= \max\{70, 65, 60\} = 70
	\end{aligned}
	\]
	\[
	\max\{90, 95, 70\} = 95
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Wind}, S\_i) \&= \min\{90, 45, 30\} = 30 \\
		\min\_i \text{Payoff}(\text{Solar}, S\_i) \&= \min\{35, 95, 25\} = 25 \\
		\min\_i \text{Payoff}(\text{Hybrid}, S\_i) \&= \min\{70, 65, 60\} = 60
	\end{aligned}
	\]
	\[
	\max\{30, 25, 60\} = 60
	...
	\[
	\begin{aligned}
		EV\_\{\text{Wind}\} \&= 0.30(90) + 0.45(45) + 0.25(30) = 27 + 20.25 + 7.5 = 54.75 \\
		EV\_\{\text{Solar}\} \&= 0.30(35) + 0.45(95) + 0.25(25) = 10.5 + 42.75 + 6.25 = 59.5 \\
		EV\_\{\text{Hybrid}\} \&= 0.30(70) + 0.45(65) + 0.25(60) = 21 + 29.25 + 15 = 65.25
	\end{aligned}
	\]
	\[
	...
	\begin{itemize}
		...
		\item \hyperlink{problem3-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	• Centralized network → 220, 140, 40, -30
	• Regional network → 180, 160, 110, 60
	...
	\begin{center}
		\begin{tabular}{lllll}
			...
			Network & Boom
			(0.20) & Stable
			(0.35) & Moderate disruptions
			(0.30) & Severe disruptions
			(0.15) \\
			\midrule
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Centralized}, S\_i) \&= \max\{220, 140, 40, -30\} = 220 \\
		\max\_i \text{Payoff}(\text{Regional}, S\_i) \&= \max\{180, 160, 110, 60\} = 180
	\end{aligned}
	\]
	\[
	\max\{220, 180\} = 220
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Centralized}, S\_i) \&= \min\{220, 140, 40, -30\} = -30 \\
		\min\_i \text{Payoff}(\text{Regional}, S\_i) \&= \min\{180, 160, 110, 60\} = 60
	\end{aligned}
	\]
	\[
	\max\{-30, 60\} = 60
	...
	\[
	\begin{aligned}
		EV\_\{\text{Centralized}\} \&= 0.20(220) + 0.35(140) + 0.30(40) + 0.15(-30) \\
		\&= 44 + 49 + 12 - 4.5 = 100.5 \\
		EV\_\{\text{Regional}\} \&= 0.20(180) + 0.35(160) + 0.30(110) + 0.15(60) \\
		\&= 36 + 56 + 33 + 9 = 134
	\end{aligned}
	\]
	\[
	...
	\begin{itemize}
		...
		\item \hyperlink{problem4-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	• Standard packaging → 900 (High), 550 (Low)
	• Premium packaging → 650 (High), 250 (Low)
	...
	\begin{itemize}
		\item Standard: High → 900 × 0.04 = 36; Low → 550 × 0.04 = 22.
		\item Premium: High → 650 × 0.07 = 45.5; Low → 250 × 0.07 = 17.5.
	\end{itemize}
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Standard}, S\_i) \&= \max\{36, 22\} = 36 \\
		\max\_i \text{Payoff}(\text{Premium}, S\_i) \&= \max\{45.5, 17.5\} = 45.5
	\end{aligned}
	\]
	\[
	\max\{36, 45.5\} = 45.5
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Standard}, S\_i) \&= \min\{36, 22\} = 22 \\
		\min\_i \text{Payoff}(\text{Premium}, S\_i) \&= \min\{45.5, 17.5\} = 17.5
	\end{aligned}
	\]
	\[
	\max\{22, 17.5\} = 22
	...
	\[
	\begin{aligned}
		EV\_\{\text{Standard}\} \&= 0.60(36) + 0.40(22) = 21.6 + 8.8 = 30.4 \\
		EV\_\{\text{Premium}\} \&= 0.60(45.5) + 0.40(17.5) = 27.3 + 7 = 34.3
	\end{aligned}
	\]
	\[
	\max\{30.4, 34.3\} = 34.3
	...
	\begin{itemize}
		...
		\item \hyperlink{problem5-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	• Basic → 1400 (High), 900 (Low)
	• Plus → 1000 (High), 700 (Low)
	• Pro → 600 (High), 300 (Low)
	...
	\begin{center}
		\begin{tabular}{ll}
			...
			Consequences & Profit in thousand USD based on subscriptions × profit per subscription \\
			\bottomrule
		\end{tabular}
	\end{center}
	...
	Profit calculations (thousand USD):
	\begin{itemize}
		\item Basic: High → 1400 × 0.03 = 42; Low → 900 × 0.03 = 27.
		\item Plus: High → 1000 × 0.05 = 50; Low → 700 × 0.05 = 35.
		\item Pro: High → 600 × 0.08 = 48; Low → 300 × 0.08 = 24.
	\end{itemize}
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Basic}, S\_i) \&= \max\{42, 27\} = 42 \\
		\max\_i \text{Payoff}(\text{Plus}, S\_i) \&= \max\{50, 35\} = 50 \\
		\max\_i \text{Payoff}(\text{Pro}, S\_i) \&= \max\{48, 24\} = 48
	\end{aligned}
	\]
	\[
	\max\{42, 50, 48\} = 50
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Basic}, S\_i) \&= \min\{42, 27\} = 27 \\
		\min\_i \text{Payoff}(\text{Plus}, S\_i) \&= \min\{50, 35\} = 35 \\
		\min\_i \text{Payoff}(\text{Pro}, S\_i) \&= \min\{48, 24\} = 24
	\end{aligned}
	\]
	\[
	\max\{27, 35, 24\} = 35
	...
	\[
	\begin{aligned}
		EV\_\{\text{Basic}\} \&= 0.55(42) + 0.45(27) = 23.1 + 12.15 = 35.25 \\
		EV\_\{\text{Plus}\} \&= 0.55(50) + 0.45(35) = 27.5 + 15.75 = 43.25 \\
		EV\_\{\text{Pro}\} \&= 0.55(48) + 0.45(24) = 26.4 + 10.8 = 37.2
	\end{aligned}
	\]
	\[
	...
	\begin{itemize}
		...
		\item \hyperlink{problem6-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	• Favorable → 6 thousand USD per shipment
	• Unfavorable → 2 thousand USD per shipment
	...
	Expected shipments:
	• Port A → 40 (Favorable), 28 (Unfavorable)
	• Port B → 32 (Favorable), 30 (Unfavorable)
	...
	\begin{center}
		\begin{tabular}{ll}
			...
			Consequences & Profit in thousand USD based on shipments × profit per shipment \\
			\bottomrule
		\end{tabular}
	\end{center}
	...
	Profit calculations (thousand USD):
	\begin{itemize}
		\item Port A: Favorable → 40 × 6 = 240; Unfavorable → 28 × 2 = 56.
		\item Port B: Favorable → 32 × 6 = 192; Unfavorable → 30 × 2 = 60.
	\end{itemize}
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Port A}, S\_i) \&= \max\{240, 56\} = 240 \\
		\max\_i \text{Payoff}(\text{Port B}, S\_i) \&= \max\{192, 60\} = 192
	\end{aligned}
	\]
	\[
	\max\{240, 192\} = 240
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Port A}, S\_i) \&= \min\{240, 56\} = 56 \\
		\min\_i \text{Payoff}(\text{Port B}, S\_i) \&= \min\{192, 60\} = 60
	\end{aligned}
	\]
	\[
	\max\{56, 60\} = 60
	...
	\[
	\begin{aligned}
		EV\_\{\text{Port A}\} \&= 0.50(240) + 0.50(56) = 120 + 28 = 148 \\
		EV\_\{\text{Port B}\} \&= 0.50(192) + 0.50(60) = 96 + 30 = 126
	\end{aligned}
	\]
	\[
	\max\{148, 126\} = 148
	...
	\begin{itemize}
		...
		\item \hyperlink{problem7-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	• Peak → 0.05 thousand USD
	• Normal → 0.04 thousand USD
	• Rainy → 0.03 thousand USD
	...
	• Small ferries → 2200 (Peak), 1600 (Normal), 1000 (Rainy)
	• Large ferries → 2600 (Peak), 1800 (Normal), 800 (Rainy)
	...
	\begin{center}
		\begin{tabular}{ll}
			...
			Consequences & Profit in thousand USD based on tickets × profit per ticket \\
			\bottomrule
		\end{tabular}
	\end{center}
	...
	Profit calculations (thousand USD):
	\begin{itemize}
		\item Small ferries: Peak → 2200 × 0.05 = 110; Normal → 1600 × 0.04 = 64; Rainy → 1000 × 0.03 = 30.
		\item Large ferries: Peak → 2600 × 0.05 = 130; Normal → 1800 × 0.04 = 72; Rainy → 800 × 0.03 = 24.
	\end{itemize}
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Small}, S\_i) \&= \max\{110, 64, 30\} = 110 \\
		\max\_i \text{Payoff}(\text{Large}, S\_i) \&= \max\{130, 72, 24\} = 130
	\end{aligned}
	\]
	\[
	\max\{110, 130\} = 130
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Small}, S\_i) \&= \min\{110, 64, 30\} = 30 \\
		\min\_i \text{Payoff}(\text{Large}, S\_i) \&= \min\{130, 72, 24\} = 24
	\end{aligned}
	\]
	\[
	\max\{30, 24\} = 30
	...
	\[
	\begin{aligned}
		EV\_\{\text{Small}\} \&= 0.40(110) + 0.35(64) + 0.25(30) = 44 + 22.4 + 7.5 = 73.9 \\
		EV\_\{\text{Large}\} \&= 0.40(130) + 0.35(72) + 0.25(24) = 52 + 25.2 + 6 = 83.2
	\end{aligned}
	\]
	\[
	\max\{73.9, 83.2\} = 83.2
	...
	\begin{itemize}
		...
		\item \hyperlink{problem8-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	• Small → 300 (High), 220 (Low)
	• Medium → 600 (High), 420 (Low)
	• Large → 900 (High), 700 (Low)
	...
	\begin{center}
		\begin{tabular}{ll}
			...
			Consequences & Profit in thousand USD = pallets × profit per pallet + fixed profit \\
			\bottomrule
		\end{tabular}
	\end{center}
	...
	Profit calculations (thousand USD):
	\begin{itemize}
		\item Small: High → 300 × 0.18 + 40 = 94; Low → 220 × 0.18 + 40 = 79.6.
		\item Medium: High → 600 × 0.12 + 25 = 97; Low → 420 × 0.12 + 25 = 75.4.
		\item Large: High → 900 × 0.08 + 10 = 82; Low → 700 × 0.08 + 10 = 66.
	\end{itemize}
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Small}, S\_i) \&= \max\{94, 79.6\} = 94 \\
		\max\_i \text{Payoff}(\text{Medium}, S\_i) \&= \max\{97, 75.4\} = 97 \\
		\max\_i \text{Payoff}(\text{Large}, S\_i) \&= \max\{82, 66\} = 82
	\end{aligned}
	\]
	\[
	\max\{94, 97, 82\} = 97
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Small}, S\_i) \&= \min\{94, 79.6\} = 79.6 \\
		\min\_i \text{Payoff}(\text{Medium}, S\_i) \&= \min\{97, 75.4\} = 75.4 \\
		\min\_i \text{Payoff}(\text{Large}, S\_i) \&= \min\{82, 66\} = 66
	\end{aligned}
	\]
	\[
	...
	\[
	\begin{aligned}
		EV\_\{\text{Small}\} \&= 0.55(94) + 0.45(79.6) = 51.7 + 35.82 = 87.52 \\
		EV\_\{\text{Medium}\} \&= 0.55(97) + 0.45(75.4) = 53.35 + 33.93 = 87.28 \\
		EV\_\{\text{Large}\} \&= 0.55(82) + 0.45(66) = 45.1 + 29.7 = 74.8
	\end{aligned}
	\]
	\[
	...
	\begin{itemize}
		...
		\item \hyperlink{problem9-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	• Rebate continues (0.55) → +20 thousand USD
	• Rebate ends (0.45) → -10 thousand USD
	...
	• Residential → 80 (Rebate), 50 (No rebate)
	• Commercial → 55 (Rebate), 40 (No rebate)
	• Mixed → 70 (Rebate), 45 (No rebate)
	...
	\begin{center}
		\begin{tabular}{ll}
			...
			Consequences & Profit in thousand USD = installs × profit per install + fixed profit \\
			\bottomrule
		\end{tabular}
	\end{center}
	...
	Profit calculations (thousand USD):
	\begin{itemize}
		\item Residential: Rebate → 80 × 0.9 + 20 = 92; No rebate → 50 × 0.9 - 10 = 35.
		\item Commercial: Rebate → 55 × 1.6 + 20 = 108; No rebate → 40 × 1.6 - 10 = 54.
		\item Mixed: Rebate → 70 × 1.2 + 20 = 104; No rebate → 45 × 1.2 - 10 = 44.
	\end{itemize}
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Residential}, S\_i) \&= \max\{92, 35\} = 92 \\
		\max\_i \text{Payoff}(\text{Commercial}, S\_i) \&= \max\{108, 54\} = 108 \\
		\max\_i \text{Payoff}(\text{Mixed}, S\_i) \&= \max\{104, 44\} = 104
	\end{aligned}
	\]
	\[
	\max\{92, 108, 104\} = 108
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Residential}, S\_i) \&= \min\{92, 35\} = 35 \\
		\min\_i \text{Payoff}(\text{Commercial}, S\_i) \&= \min\{108, 54\} = 54 \\
		\min\_i \text{Payoff}(\text{Mixed}, S\_i) \&= \min\{104, 44\} = 44
	\end{aligned}
	\]
	\[
	\max\{35, 54, 44\} = 54
	...
	\[
	\begin{aligned}
		EV\_\{\text{Residential}\} \&= 0.55(92) + 0.45(35) = 50.6 + 15.75 = 66.35 \\
		EV\_\{\text{Commercial}\} \&= 0.55(108) + 0.45(54) = 59.4 + 24.3 = 83.7 \\
		EV\_\{\text{Mixed}\} \&= 0.55(104) + 0.45(44) = 57.2 + 19.8 = 77
	\end{aligned}
	\]
	\[
	...
	\begin{itemize}
		...
		\item \hyperlink{problem10-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	• Low fuel → 8 thousand USD
	• Medium fuel → 5 thousand USD
	• High fuel → 2 thousand USD
	...
	Expected charters:
	• Narrow-body → 20 (Low), 18 (Medium), 15 (High)
	• Wide-body → 14 (Low), 12 (Medium), 9 (High)
	...
	Profit calculations (thousand USD):
	\begin{itemize}
		\item Narrow-body: Low → 20 × 8 + 30 = 190; Medium → 18 × 5 + 30 = 120; High → 15 × 2 + 30 = 60.
		\item Wide-body: Low → 14 × 8 + 50 = 162; Medium → 12 × 5 + 50 = 110; High → 9 × 2 + 50 = 68.
	\end{itemize}
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Narrow}, S\_i) \&= \max\{190, 120, 60\} = 190 \\
		\max\_i \text{Payoff}(\text{Wide}, S\_i) \&= \max\{162, 110, 68\} = 162
	\end{aligned}
	\]
	\[
	\max\{190, 162\} = 190
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Narrow}, S\_i) \&= \min\{190, 120, 60\} = 60 \\
		\min\_i \text{Payoff}(\text{Wide}, S\_i) \&= \min\{162, 110, 68\} = 68
	\end{aligned}
	\]
	\[
	\max\{60, 68\} = 68
	...
	\[
	\begin{aligned}
		EV\_\{\text{Narrow}\} \&= 0.30(190) + 0.45(120) + 0.25(60) = 57 + 54 + 15 = 126 \\
		EV\_\{\text{Wide}\} \&= 0.30(162) + 0.45(110) + 0.25(68) = 48.6 + 49.5 + 17 = 115.1
	\end{aligned}
	\]
	\[
	\max\{126, 115.1\} = 126
	...
	\begin{itemize}
		...
		\item \hyperlink{problem11-c1}{(C1) Formulating the decision problem for an optimal choice}
	\end{itemize}
	...
	• Strong tourism season (0.40) → +30 thousand USD
	• Normal season (0.35) → +10 thousand USD
	• Stormy season (0.25) → -15 thousand USD
	...
	• Strong → 0.7 thousand USD
	• Normal → 0.5 thousand USD
	• Stormy → 0.2 thousand USD
	...
	• Frozen fillet → 120 (Strong), 100 (Normal), 80 (Stormy)
	• Fresh export → 90 (Strong), 110 (Normal), 70 (Stormy)
	...
	\begin{center}
		\begin{tabular}{ll}
			...
			Consequences & Profit in thousand USD = crates × profit per crate + fixed profit \\
			\bottomrule
		\end{tabular}
	\end{center}
	...
	Profit calculations (thousand USD):
	\begin{itemize}
		\item Frozen fillet: Strong → 120 × 0.7 + 30 = 114; Normal → 100 × 0.5 + 10 = 60; Stormy → 80 × 0.2 - 15 = 1.
		\item Fresh export: Strong → 90 × 0.7 + 30 = 93; Normal → 110 × 0.5 + 10 = 65; Stormy → 70 × 0.2 - 15 = -1.
	\end{itemize}
	...
	\[
	\begin{aligned}
		\max\_i \text{Payoff}(\text{Frozen}, S\_i) \&= \max\{114, 60, 1\} = 114 \\
		\max\_i \text{Payoff}(\text{Fresh}, S\_i) \&= \max\{93, 65, -1\} = 93
	\end{aligned}
	\]
	\[
	\max\{114, 93\} = 114
	...
	\[
	\begin{aligned}
		\min\_i \text{Payoff}(\text{Frozen}, S\_i) \&= \min\{114, 60, 1\} = 1 \\
		\min\_i \text{Payoff}(\text{Fresh}, S\_i) \&= \min\{93, 65, -1\} = -1
	\end{aligned}
	\]
	\[
	\max\{1, -1\} = 1
	...
	\[
	\begin{aligned}
		EV\_\{\text{Frozen}\} \&= 0.40(114) + 0.35(60) + 0.25(1) = 45.6 + 21 + 0.25 = 66.85 \\
		EV\_\{\text{Fresh}\} \&= 0.40(93) + 0.35(65) + 0.25(-1) = 37.2 + 22.75 - 0.25 = 59.7
	\end{aligned}
	\]
	\[
