...
\begin{document}
	...
	\begin{multicols}{2}
		...
		A firm must choose one strategy among $A_1,A_2,A_3$ under uncertain market conditions.
		...
		\begin{itemize}
			\item Step 1: Retain the top two alternatives according to Expected Value (EV).
			\item Step 2: Within that subset, apply the Minimax Regret criterion and choose one.
		\end{itemize}
		...
		\begin{itemize}
			\item EV ranking: \par $A_2>A_3>A_1$.
			\item Maximax ranking:\par $A_3>A_1>A_2$.
			\item Maximin ranking: \par$A_2>A_1>A_3$.
			\item Minimax Regret ranking:\par $A_2<A_1<A_3$.
		\end{itemize}
		
		\textbf{C9 solution (apply filters)}\par
		\begin{itemize}
			\item \textbf{(Step 1)} EV top 2: $\{A_2,A_3\}$.
			\item \textbf{(Step 2)} Minimax Regret keeps $\{A_2\}$.
			\item Final decision: $A_2$.
		\end{itemize}
		...
		An insurance broker considers six portfolio mixes $Y_1$ to $Y_6$ under four climate-risk states. 
		Payoffs are in thousand USD.
		
		\begin{center}
			\begin{tabular}{l c c c c c c c}
				\hline
				State & Prob. & $Y_1$ & $Y_2$ & $Y_3$ & $Y_4$ & $Y_5$ & $Y_6$ \\
				\hline
		...
		After obtaining the C10 summary and computing the C8 results, apply the following structured C9 decision procedure:
		\begin{itemize}
			\item Step 1: Retain the top four alternatives according to Expected Value (EV).
		...
		\textbf{C8 solution (ranking and recommendations)}\par
		\begin{itemize}
			\item EV ranking: \par $Y_4>Y_2>Y_5>Y_6>Y_1>Y_3$
			\item Maximax ranking: \par $Y_3>Y_1>Y_5>Y_2>Y_6>Y_4$.
			\item Maximin ranking: \par $Y_4>Y_5>Y_2>Y_6>Y_1>Y_3$.
			\item Minimax Regret ranking: \par $Y_2=Y_5<Y_6<Y_4<Y_1<Y_3$.
		\end{itemize}
		
		\textbf{C9 solution (apply filters)}\par
		\begin{itemize}
			\item \textbf{(Step 1)} EV top 3: $\{Y_4,Y_2,Y_5,Y_6\}$.
			\item \textbf{(Step 2)} Minimax Regret keeps $\{Y_2,Y_5\}$.
			\item \textbf{(Step 3)} Maximin selects $Y_5$.
			\item Final decision: $Y_5$.
		\end{itemize}
